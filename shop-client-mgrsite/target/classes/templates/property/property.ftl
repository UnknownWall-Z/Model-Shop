...
<script type="text/javascript">
	...
	function addValue(){
        ...
        var vdt = $("#valueDivTemplate");

	    $("#valueDivTemplate >>input").prop("name","value"+(++sequence));

        vdt.css("display","line");

		$("#valueDiv").append(vdt.html());
	}
	...
	function addItem() {
        var $input = $("input[name=value"+sequence+"]");
        var $hidden = $("input[name='catalogProperty.id']");
        if($input.val()){
            $.ajax({
                url:"/catalogPropertyValue/add",
                data:{
                    value:$input.val(),
                    catalogPropertyId:$hidden.val(),
                    sequence:sequence
                },
                type:"GET",
	...
	function addCatalogProperty(propertyId){
		...
		$("#propertyFormContent").load("/catalogPropertyEdit/add?id="+propertyId+"&catalogId="+sNodes[0].id);
	...
	function saveCatalogProperty(){
		...
		form.ajaxSubmit(function(data) {
			if (data) {
				$.messager.popup("提示", "操做成功!");
				window.location.reload();
			} else {
				$.messager.alert("提示", "操做失败!");
			}
		});
	}
...
<!-- 属性值的模板 -->
<div id="valueDivTemplate" style="display:none">
